...
(function detectPlatform() {
    ...
    if (window.isIOS) {
        ...
        console.log("Modo: iOS App - Aplicando parche Notch");

        // 1. FORZAR viewport-fit=cover (Vital para que el iPhone reconozca el notch)
        ...
        // Si no tiene viewport-fit=cover, se lo añadimos
        if (!meta.content.includes('viewport-fit=cover')) {
            meta.content = "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover";
        }

        // 2. INYECTAR CSS ESPECÍFICO PARA EMPUJAR LA CABECERA
        const style = document.createElement('style');
        style.innerHTML = `
            /* Variable CSS para la altura del notch */
            :root {
            ...
            /* 2. SI TU CABECERA ES FIJA (position: fixed) */
            /* Esto busca etiquetas comunes de cabecera. Si tu barra blanca tiene una ID específica, añádela aquí */
            body.platform-ios header, 
            body.platform-ios .header,
            body.platform-ios nav,
            body.platform-ios .top-bar {
                padding-top: var(--safe-top) !important;
                height: auto !important; /* Permitir que crezca */
            }
            
            /* Ajuste específico para el contenedor principal si es necesario */
            body.platform-ios #app-view {
                position: relative; 
                z-index: 1;
            }
        `;
...
function goToLogin() {
    ...
    document.getElementById('auth-container').classList.add('fade-in');
    window.scrollTo(0, 0);
}
...
function showLanding() {
    ...
    // Mostrar Landing y Header
    document.getElementById('landing-view').classList.remove('hidden');
    document.getElementById('public-header').classList.remove('hidden');

    // Limpiar estilos body
    document.body.classList.remove('bg-[#fafaf9]', 'is-admin', 'is-profesor');
    window.scrollTo(0, 0);
}

// --- LISTENER DE AUTENTICACIÓN UNIFICADO ---
// Este reemplaza a los dos anteriores que tenías en conflicto
client.auth.onAuthStateChange((event, session) => {
    ...
    if (session) {
        ...
        pubHeader.classList.add('hidden'); // El header público se va
        auth.classList.add('hidden');
        app.classList.remove('hidden'); // Entramos a la app

        currentUser = session.user;
        initApp();
        loadProfileCard();
    }
    // Caso 2: Usuario NO Logueado (o Logout)
    else {
        // Importante: Al cargar, si no hay sesión, mostramos Landing por defecto.
        // NO mostramos login directo salvo que el usuario ya estuviera allí (lógica simple: Landing first)

        app.classList.add('hidden');
        currentUser = null;
        document.body.classList.remove('is-admin');

        // Si el evento es explícitamente SALIR, volvemos a la Landing
        if (event === 'SIGNED_OUT') {
            showLanding();
            // Limpiamos caché básica
            allUsersCache = [];
            allClasesCache = [];
        } else {
            // Carga inicial sin sesión: Aseguramos que se vea la landing
            // Verificamos que no estemos ya en el login (por si acaso)
...
window.addEventListener('scroll', () => {
    ...
    if (window.scrollY > 50) {
        ...
        header.classList.add('bg-gray-900/90', 'backdrop-blur-md', 'py-2', 'shadow-md');
    } else {
        header.classList.add('bg-transparent', 'py-4');
        header.classList.remove('bg-gray-900/90', 'backdrop-blur-md', 'py-2', 'shadow-md');
    }
});
...
// Modificar función logout para que no recargue la página, sino que use el flujo de estados
async function logout() {
    await client.auth.signOut();
    // No necesitamos location.reload(), el onAuthStateChange manejará la redirección a showLanding()
}
